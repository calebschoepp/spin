...
mod otel {
    ...
    impl From<wasi_metrics::InstrumentationScope> for opentelemetry::InstrumentationScope {
        fn from(value: wasi_metrics::InstrumentationScope) -> Self {
            let attributes: Vec<opentelemetry::KeyValue> =
                value.attributes.into_iter().map(Into::into).collect();
            opentelemetry::InstrumentationScope::builder(Cow::Owned(value.name))
                .with_version(Cow::Owned(value.version))
                .with_schema_url(Cow::Owned(value.schema_url))
                .with_attributes(attributes)
                .build()
        }
    }
    ...
    impl From<wasi_metrics::TemporalityT> for opentelemetry_sdk::metrics::Temporality {
        fn from(value: wasi_metrics::TemporalityT) -> Self {
            use opentelemetry_sdk::metrics::Temporality as OT;
            use wasi_metrics::TemporalityT as T;
            match value {
                T::Delta => OT::Delta,
    ...
    impl From<wasi_tracing::InstrumentationScope> for opentelemetry::InstrumentationScope {
